...
public class GameOntology extends Ontology implements GameVocabulary {
	...
	private GameOntology() {
		...
		try {
			// Add the Defect and Cooperate actions
			add(new AgentActionSchema(DEFECT), DefectAction.class);
			add(new AgentActionSchema(COOPERATE), CooperateAction.class);
			// Add the Start action
			final AgentActionSchema aas = new AgentActionSchema(START);
